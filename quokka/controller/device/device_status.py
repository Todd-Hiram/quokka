...
def get_device_status(device):
    ...
    if device["os"] in {"ios", "iosxe", "nxos-ssh"} and device["transport"] == "napalm":
        ...
        try:
            ...
            info = f"!!! Exception in monitoring device, get environement: {repr(e)}"
            log_console(info)
            log_event(
        ...
        try:
            ...
            log_console(info)
            log_event(
                str(datetime.now())[:-3], "devic", device["name"], "SEVERE", info
            )
            result = "failed"
        ...
        if result != "success":
            ...
            if response_time:
                device_status["response_time"] = int(response_time 8 1000)
